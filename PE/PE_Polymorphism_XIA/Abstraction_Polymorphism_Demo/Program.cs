...
namespace Abstraction_Polymorphism_Demo
{
    class Program
    {
        ...
        private static void ShapesDemo()
        {
            ...
            do
            {
                choice = GetPromptedInput(
                    "What type of shape do you want (circle, sector, square, rectangle or Q to quit)?").ToUpper();
                switch (choice)
                {
                    ...
                    case "CIRCLE":
                        ...
                        //In this line the newCircle is implicitly casted
                        //from a Circle type to a Shape type.
                        shapes.Add(newCircle);
                        break;

                    //Get the radius and the angle degree for a new sector
                    //and add it to the list
                    case "SECTOR":
                        radius = GetPromptedPositiveDouble("What is the radius?");
                        double degree = GetPromptedPositiveDouble("What is the degree?");
                        shapes.Add(new Sector(radius, degree));
                        break;
                    ...
                    //Get the length and width for a new rectangle and add it to the list
                    case "RECTANGLE":
        ...
        private static void PetsDemo()
        {
            ...
            for (int i = 0; i < pets.Length; i++)
            {
                ...
                {
                    pets[i] = new Chick(names[i], DateTime.Today);
                }
                else //else if i % 5 == 4
                {
                    pets[i] = new Hamster(names[i], DateTime.Today);
                }
                pets[i].Speak();
            }
            Console.WriteLine();
            ...
            for (int i = 0; i < pets.Length; i++)
            {
                ...
                else if (pets[i] is Hamster)
                {
                    ((Hamster)pets[i]).RunRoller(5);
                }
